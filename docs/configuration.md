...
The packaged `/skill:pi-card-callibration` can help review a small sample of past user messages. It does not run automatically. Invocation is not consent: before any session JSONL is read, the skill states the configured agent sessions root, current-project filter, inclusive date range (default suggestion: the last 7 calendar days), file/candidate caps, and intended extraction. Raw JSONL stays local, but redacted excerpts reviewed in chat are sent to the active Pi model provider; no session text is sent to TypeSafe by this skill. The skill requires explicit approval for both the read scope and excerpt sharing. The miner is pinned to `${PI_CODING_AGENT_DIR:-~/.pi/agent}/sessions`, rejects symlinked roots, and bounds output to 20 candidates. Redaction is best-effort; review it carefully. Only examples you approve can be added to `pi-card.json`. Pi Card sends configured examples to TypeSafe alongside future eligible messages it routes. Each `stop` example needs separate confirmation. Invalid or full configs are left unchanged; the stop confidence/probability gate is unchanged.
